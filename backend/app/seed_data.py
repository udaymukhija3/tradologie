...
def seed_database(db: Session) -> None:
    ...
    db.add(
        VoiceAgent(
            ...
            name="Trade Support Agent",
            provider="openai",
            voice="marin",
            system_prompt="Use workspace tools for business truth and require confirmation before writes.",
        )
    )
    db.flush()
    seeded = [
    ...
    for index, (display_id, product, quantity, unit, destination, status) in enumerate(seeded):
        confirmation = Confirmation(
            id=f"seed_confirm_{index + 1}",
            workspace_id=DEMO_WORKSPACE_ID,
            user_id=DEMO_ADMIN_ID,
            session_id="seed",
            action="create_enquiry",
            payload_hash=f"seed-{index + 1}",
            payload={"product": product, "quantity": quantity, "unit": unit, "destination": destination},
            status=ConfirmationStatus.CONSUMED,
            idempotency_key=f"seed-enquiry-{index + 1}",
        ...
        db.add(
            Enquiry(
                id=f"seed_enquiry_{index + 1}",
                workspace_id=DEMO_WORKSPACE_ID,
                buyer_id=DEMO_ADMIN_ID,
